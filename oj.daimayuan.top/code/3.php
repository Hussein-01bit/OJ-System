...
<html lang="zh-cn">
	<head>
		...
		<meta name="viewport" content="width=device-width, initial-scale=1" />
				<title>登录 - Daimayuan Online Judge</title>
...
<form id="form-login" class="form-horizontal" method="post">
  ...
  <div class="form-group">
    <div class="col-sm-offset-2 col-sm-3">
      <button type="submit" id="button-submit" class="btn btn-secondary" onclick="login();">提交</button>
    </div>
  </div>
</form>
...
<script type="text/javascript">
function login() {
	var input_username = document.querySelector("#input-username").value;
	var input_password = document.querySelector("#input-password").value;
	var pair_username_and_password = localStorage.getItem("uoj-user_" + input_username + "_password");
	if (localStorage.getItem("uoj-username") != null) 
		alert("已登录账号，请登出后再登录。");
	else if (pair_username_and_password == null) 
		alert("不存在该用户，请先注册。");
	else if (localStorage.getItem("uoj-user_" + input_username + "_banned") != null) 
		alert("该用户已被封停，请联系管理员。");
	else if (input_password != pair_username_and_password) {
		if (confirm("用户名或密码错误，忘记密码？")) {
			alert("暂不开放");
			// window.location.href = "fand-password";
		}
	}
	else {
		localStorage.setItem("uoj-username", input_username);
		history.back();
