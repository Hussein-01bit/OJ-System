...
<html lang="zh-cn">
	<head>
		...
		<meta name="viewport" content="width=device-width, initial-scale=1" />
				<title>注册 - Daimayuan Online Judge</title>
		...
		<script src="http://oj.daimayuan.top/js/md5.min.js"></script>				
				<!-- Bootstrap dialog -->
		<link type="text/css" rel="stylesheet" href="http://oj.daimayuan.top/css/bootstrap-dialog.min.css" />		<script src="http://oj.daimayuan.top/js/bootstrap-dialog.min.js"></script>				
...
<form id="form-register" class="form-horizontal">
	<div id="div-username" class="form-group">
		...
		<div class="col-sm-3">
			<input type="text" class="form-control" id="input-username" name="username" placeholder="输入用户名" maxlength="20" />
	...
	<div id="div-password" class="form-group">
		...
		<div class="col-sm-3">
			<input type="password" class="form-control" id="input-password" name="password" placeholder="输入密码" maxlength="20" />
			<input type="password" class="form-control top-buffer-sm" id="input-confirm_password" placeholder="重新输入密码" maxlength="20" />
			<span class="help-block" id="help-password"></span>
		</div>
	</div>
	<div class="form-group">
		<div class="col-sm-offset-2 col-sm-3">
			<button type="submit" id="button-submit" class="btn btn-secondary" onclick="register();">提交</button>
		</div>
	</div>
</form>
...
function register() {
	...
	var cpassword = document.getElementById("input-confirm_password").value;
	if (localStorage.getItem("uoj-user_" + username + "_password") != null) 
		alert("该用户名已被人使用了。");
		...
		localStorage.setItem("uoj-user_" + username + "_password", password);
		localStorage.setItem("uoj-user_" + username + "_problems", 0);
		localStorage.setItem("uoj-user_" + username + "_maxim", "");
		var cnt = 1;
		for ( ; ; cnt++) 
			if (localStorage.getItem("uoj-user-id=" + cnt) == null) 
				break;
		localStorage.setItem("uoj-user-id=" + cnt, username);
		localStorage.setItem("uoj-user_" + username + "_rating", 1500);
		alert("欢迎你！" + username + "，你已成功注册。");
		if (cnt == 1) {
			localStorage.setItem("uoj-super_gler-user", username);
			localStorage.setItem("uoj-user_" + username + "_gler", 1);
			alert("系统已将您自动升级为超级管理员。");
		}
	}
}
</script>
				...
				<ul class="list-inline"><li class="list-inline-item">Daimayuan Online Judge</li></ul>
								<p>服务器时间: 2025-01-22 13:47:09 | <a href="https://github.com/UniversalOJ/UOJ-System" target="_blank">开源项目</a></p>
			</div>
					</div>
